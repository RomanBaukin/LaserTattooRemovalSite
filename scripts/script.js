...
const createCardTemplate = () => {
  ...
  const numberPet = randomNumberPet();

  h3.classList.add("card__title");
  h3.innerText = petsLibrary[numberPet].name;
  div.classList.add("card__image");
  div.style.backgroundImage = `url('${petsLibrary[numberPet].img}')`;
...
createItemCarousel(ITEM_LEFT);
createItemCarousel(ITEM_ACTIVE);
createItemCarousel(ITEM_RIGHT);
...
mobileMenu.addEventListener('click', (event) => {
  ...
  if (target.matches('.mobile-menu__items li a')) {
    openCloseMenu();
  }
})